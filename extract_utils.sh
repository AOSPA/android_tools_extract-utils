...
# SPDX-FileCopyrightText: 2017-2020 Paranoid Android
...
function write_blueprint_header() {
    ...
    local SETUP_MAKEFILES_PATH="device/$VENDOR/$([ -z "$IS_QTI_COMPONENT" ] && echo ${DEVICE} || echo ${COMPONENT})"
    if [ -n "$KERNEL_VERSION" ]; then
        SETUP_MAKEFILES_PATH+="/$KERNEL_VERSION"
    fi
    SETUP_MAKEFILES_PATH+="/setup-makefiles.sh"

    cat <<EOF >>"$1"
// Automatically generated file. DO NOT MODIFY
//
// This file is generated by $SETUP_MAKEFILES_PATH
...
EOF
    ...
    [ "$COMMON" -eq 1 ] && local DEVICE="$DEVICE_COMMON"
    [ "$COMMON" -eq 1 ] && local VENDOR="${VENDOR_COMMON:-$VENDOR}"
    local SOONG_NAMESPACE="vendor/$VENDOR/$([ -z "$IS_QTI_COMPONENT" ] && echo ${DEVICE} || echo ${COMPONENT})"
    if [ -n "$KERNEL_VERSION" ]; then
        SOONG_NAMESPACE+="/$KERNEL_VERSION"
    fi
    cat <<EOF >>"$PRODUCTMK"
