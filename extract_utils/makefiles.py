...
def write_elfs_package(
    ...
    if is_bin and has_rust_deps:
        (
            builder.set_rule_name('rust_prebuilt_binary')
            .name(package_name)
            .stem(stem)
            .owner()
            .targets(files, machines, depses, rust_depses)
    ...
    # Non-rust binaries / libraries with rust dependencies are not supported
    # Requires implementing rust_prebuilt_ffi_shared
    assert not has_rust_deps, (
        f'{file.dst}: rust prebuilt shared libraries are not supported yet'
    )
